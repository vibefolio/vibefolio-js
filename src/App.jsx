...
import { AppImageCard } from "./components/common/AppImageCard";
import {
  Avatar,
  AvatarFallback,
  AvatarImage,
...
function App() {
  ...
  const fetchApi = async () => {
    ...
    const res = await axios.get(`${API_URL}&page=2&query=korean`);
    console.log("res: ", res);
  ...
  return (
    <div className="w-full">
      ...
      <main className="w-full flex flex-col items-center py-6">
        ...
        <AppStickyMenu />
        {/* IMAGE LIST */}
        <section className="w-full grid grid-cols-5 gap-6 mt-4 px-20">
          {images.map((image, index) => {
            return (
              <Dialog key={image.index}>
                <DialogTrigger>
                  <AppImageCard image={image} />
                </DialogTrigger>
                <DialogContent>
                  <DialogHeader>
